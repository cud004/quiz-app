  ...
  async getTagSuggestions(req, res) {
    try {
      ...
      if (tagIds.length > 0) {
        ...
        counts.forEach(c => { tagQuestionCounts[c._id.toString()] = c.count; });
      }

      // Map dữ liệu trả về
      const suggestions = stats.map(stat => {
        const percent = stat.totalAnswered > 0 ? Math.round((stat.correctAnswered / stat.totalAnswered) * 100) : 0;
        ...
        return {
          ...
          percent,
          status,
          totalQuestionsInTag: tagQuestionCounts[stat.tag?._id?.toString()] || 0
        };
      });
